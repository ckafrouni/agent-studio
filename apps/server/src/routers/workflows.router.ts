...
router.post("/messages", async (c) => {
  const { prompt, workflow: workflowName = 'vector-rag' } = await c.req.json<{ prompt: string, workflow?: string }>();

  if (!prompt) {
    return c.json({ error: "Missing prompt" }, 400);
  }

  const selectedWorkflow = workflowName === 'web-search-rag' 
    ? webSearchRagWorkflow
    : vectorRagWorkflow; // Default to vector-rag

  console.log(`Using workflow: ${workflowName}`); // Log which workflow is used
