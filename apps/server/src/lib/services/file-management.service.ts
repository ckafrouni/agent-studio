...
const extractFilename = (
  sourceUriOrFilename: string | undefined
): string | undefined => {
...
class FileManagementService {
  async uploadAndProcessDocument(
    ...
    try {
      await fileStorageService.uploadFileToS3(
        fileBuffer,
        originalFilename,
        fileType
      );
      ...
      if (message.includes("S3")) {
        throw new Error(
          `Failed to upload ${originalFilename} to storage: ${message}`
        );
      } else {
        throw new Error(
          `Failed to process file ${originalFilename}: ${message}`
        );
      }
    }
  }
  ...
  async deleteDocument(source: string) {
    ...
    const dbDeleteResult = await documentService.deleteVectorDataBySource(
      filename
    );
